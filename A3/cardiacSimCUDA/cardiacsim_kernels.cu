...
__global__ void simulate_version4(const double alpha, const int n, const int m, const double kk, const double dt, const double a, const double epsilon, const double M1, const double M2, const double b, double *E_1D, double *E_prev_1D, double *R_1D, int WIDTH)
{
	...
	// if (row >= 1 && row <= m && col >= 1 && col <= n) {
	// 	//tempE[l_row][l_col] = E_1D[index];
	// 	//tempE_prev[l_row][l_col] = E_prev_1D[index];
	// 	tempR[l_row][l_col] = R_1D[index];
	// }

	//Load boundries from Right Left
	// if (l_col == 1) {
	// 	//tempE[l_row][l_col - RADIUS] = E_1D[row * WIDTH + (col - RADIUS)];
	// 	//tempE[l_row][l_col + TILE_DIM] = E_1D[row * WIDTH + (col + TILE_DIM)];

	// 	tempR[l_row][l_col - RADIUS] = R_1D[row * WIDTH + (col - RADIUS)];
	// 	tempR[l_row][l_col + TILE_DIM] = R_1D[row * WIDTH + (col + TILE_DIM)];
		
	// 	//tempE_prev[l_row][l_col - RADIUS] = E_prev_1D[row * WIDTH + (col - RADIUS)];
	// 	//tempE_prev[l_row][l_col + TILE_DIM] = E_prev_1D[row * WIDTH + (col + TILE_DIM)];
	// }

	// // Load boudries From top bottom.
	// if (l_row == 1) {

	// 	//tempE[l_row - RADIUS][l_col] = E_1D[(row - RADIUS)* WIDTH + col];
	// 	//tempE[l_row + TILE_DIM][l_col] = E_1D[(row + TILE_DIM)* WIDTH + col];

	// 	tempR[l_row - RADIUS][l_col] = R_1D[(row - RADIUS)* WIDTH + col];
	// 	tempR[l_row + TILE_DIM][l_col] = R_1D[(row + TILE_DIM)* WIDTH + col];

	// 	//tempE_prev[l_row - RADIUS][l_col] = E_prev_1D[(row - RADIUS)* WIDTH + col];
	// 	//tempE_prev[l_row + TILE_DIM][l_col] = E_prev_1D[(row + TILE_DIM)* WIDTH + col];
	...
	if (row >= 1 && row <= m && col >= 1 && col <= n)
	{
		...
		// PDE
		//E_1D[index] = E_prev_1D[index] + alpha * (E_prev_1D[row * WIDTH + (col + 1)] + E_prev_1D[row * WIDTH + (col - 1)] - 4 * E_prev_1D[index] + E_prev_1D[(row + 1) * WIDTH + col] + E_prev_1D[(row - 1) * WIDTH + col]);
	...
	// if (l_row <= TILE_DIM && l_col <= TILE_DIM) {
	// 	// =======================           Shared                ========================
...
int main(int argc, char **argv)
{
	...
	*/
	...
	cout << "Max: " << mx << " L2norm: " << l2norm << " (GPU)" <<endl;
