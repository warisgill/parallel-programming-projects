...
int main(void)
{
  ...
  // cudaMemcpy(device_array_a, array_a, num_bytes, cudaMemcpyHostToDevice);
  // cudaMemcpy(device_array_b, array_b, num_bytes, cudaMemcpyHostToDevice);
  ...
  // cudaMemcpy(array_c, device_array_c, num_bytes, cudaMemcpyDeviceToHost);


  cudaDeviceSynchronize(); // Needed because kernel launch is async.
