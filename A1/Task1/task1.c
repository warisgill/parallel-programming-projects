...
double **getGaussian(int width, int height, double sigma)
{
    ...
    #pragma omp parallel for collapse(2) reduction(+: sum) private(i, j)
    for (i = 0; i < height; i++)
    {
        for (j = 0; j < width; j++)
        {
...
// helper function
void applyFilterHelper(double *** newImage, double ***image, double **filter, int filterWidth, int filterHeight, int i,int j,int d){
    int h,w;
    // #pragma omp parallel
    // {
        // #pragma omp for collapse(2) private(h, w)
...
double ***applyFilter(double ***image, double **filter, int width, int height, int filterWidth, int filterHeight)
{
    ...
    #pragma omp parallel for collapse(3) private(d, i, j)
    for (d = 0; d < 3; d++)
    {
        for (i = 0; i < newImageHeight; i++)
        {
            ...
            {
                applyFilterHelper(newImage,image,filter,filterWidth,filterHeight,i,j,d);
            }
        }
    }
...
void averageRGB(double ***image, int width, int height)
{
    ...
    #pragma omp parallel
    {
        #pragma omp for collapse(2) private(i, j) reduction(+ \
                                                    : sum[0]) 
        for (j = 0; j < height; j++)
        {
            for (k = 0; k < width; k++)
